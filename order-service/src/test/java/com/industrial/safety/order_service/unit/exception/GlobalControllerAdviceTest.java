package com.industrial.safety.order_service.unit.exception;

import com.industrial.safety.order_service.exception.GlobalControllerAdvice;
...
class GlobalControllerAdviceTest {
    ...
    void resourceNotFound_returns404() {
        ...
        ProblemDetail pd = advice.handleResourceNotFoundException(
                new ResourceNotFoundException("Order", "id", 1L), request);
    ...
    void methodArgumentNotValid_returns400() {
        ...
        given(bindingResult.getFieldErrors())
                .willReturn(List.of(new FieldError("orderRequest", "userId", "must not be blank")));
    ...
    @Test
    @DisplayName("InvalidCouponException -> 422")
    void invalidCoupon_returns422() {
        ProblemDetail pd = advice.handleInvalidCoupon(new InvalidCouponException("cupón vencido"));
    ...
    void illegalArgument_returns400() {
        ProblemDetail pd = advice.handleIllegalArgument(new IllegalArgumentException("bad"));
        assertThat(pd.getStatus()).isEqualTo(HttpStatus.BAD_REQUEST.value());
    }
    ...
    void illegalState_returns409() {
        ProblemDetail pd = advice.handleIllegalState(new IllegalStateException("conflict"));
